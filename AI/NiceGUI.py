  ...
  if (joystick && stick) {
    ...
    document.getElementById("open-camera").addEventListener("click", function () {
        ...
        if (!cameraOpen) {
            ...
            videoDiv.innerHTML =
                '<img id="camera-stream" src="http://10.226.53.199:8000/video" ' +
...
document.getElementById("cam2").addEventListener("click", function() {
  ...
  socket.onmessage = (event) => {
    ...
    async def handle_turn_on():
      await turn_on_all()
      await send_command("test FR 2000")
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  function setActiveTask(selectedId, model) {
    ...
    // reset all buttons
    tasks.forEach(btn => btn.classList.remove('active'));1
